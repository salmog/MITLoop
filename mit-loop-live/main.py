...
templates = Jinja2Templates(directory="mit-loop-live/templates")
                    ...
                    if is_breakout and is_high_vol:
                        ...
                        if weeks_count >= 4:
                            ...
                            if not has_touched_early and not dropped_below_trend:
                                ...
                                if distance_pct > 0:
                                    logic_desc = (f"Confirmed institutional breakout {weeks_count} weeks ago with a volume spike. "
                                    ...
                                    discovered_setups.append({
                                        ...
                                        "target": float(res_line), "dist": float(distance_pct), "weeks": int(weeks_count),
                                        "logic": logic_desc, "atr": float(current_row['atr'])
                                    })
                                break
        except: continue
                    ...
                    if risk_per_share > 0:
                        ...
                        if shares > 0 and total_cost <= cash_available:
                            order_request = LimitOrderRequest(
                                symbol=setup["symbol"], qty=shares, side=OrderSide.BUY,
                                time_in_force=TimeInForce.GTC, limit_price=round(setup["target"], 2),
                                order_class=OrderClass.OTO, stop_loss=StopLossRequest(stop_price=round(stop_loss_px, 2))
                            )
                            client.submit_order(order_request)
                            log_system_action(f"✅ [{strat_name}] Rested OTO Limit: {shares} shares of {setup['symbol']} at ${setup['target']:.2f}")
                        elif total_cost > cash_available:
